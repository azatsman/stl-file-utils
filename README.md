...
The source code of some of the tools depends on `boost::program_options` include files and the library. On a Debian-based Linux system the corresponding package is called `libboost-program-options-dev`.

Currentl only `Makefile` is provided in the hope that it works on most Linux systems.
